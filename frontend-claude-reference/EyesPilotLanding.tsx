...
// ⚠️ Remplacer par votre vraie URL Calendly personnelle
const CALENDLY_URL = "https://calendly.com/VOTRE-LIEN-ICI";
...
/* -------------------------------------------------------------------------- */
/*  Logo — œil avec pupille volant F1 qui suit le curseur                     */
/* -------------------------------------------------------------------------- */
...
const EyesPilotLogo = ({ size = 28, interactive = true, className = "" }: LogoProps) => {
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent) => {
      ...
      const shiftX = (dx / dist) * maxShift * factor;
      const shiftY = (dy / dist) * maxShift * factor;
